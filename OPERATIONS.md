...
Complete the bounded root-chunk phase before any singleton request starts. Collect only roots whose complete provider chains produced count mismatches; every other terminal error must still abort siblings immediately. Run one global calibration wave containing the first `min(request-wide concurrency, total mismatched texts)` real singletons through their full provider fallback chains and retain its results. Then compare only the remaining singleton work with the remaining shared deadline: `ceil(remaining singleton texts / request-wide concurrency) × observed calibration-wave duration`. If that work cannot fit, fail after calibration and before any further singleton call with the privacy-safe classified error `count-mismatch singleton recovery cannot fit the remaining request deadline`. Otherwise, process the remaining affected texts through the same globally bounded singleton queue, preserving result order and the full provider fallback chain for every text.
