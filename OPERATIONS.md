...
Provider count-mismatch recovery is deliberately narrower than ordinary fallback. Only when every attempted provider returns a count mismatch for the same multi-text chunk does the SaaS bisect it for up to three binary levels. The default eight-text provider chunk can therefore reach singletons, while a larger operator-configured chunk remains bounded to at most 15 provider-chain runs. A singleton or depth-limit mismatch remains terminal. Timeouts, authentication failures, quota/rate limits, U+0000 output, and other malformed responses never enter this isolation path. The root warning records only the provider chain and batch size, and a terminal error records only provider/error-class/count metadata — never source or translated text, URLs, or credentials.
