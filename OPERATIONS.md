...
Since WordPress plugin v0.12.7, the authenticated SaaS project owns source language, target languages, automatic redirect, AI disclosure, and automatic-translation policy. WordPress applies them only from a complete, monotonic runtime snapshot and displays source, targets, and redirect as read-only mirrors after the first snapshot. A key or backend change carries the previous mirrors only as bootstrap input until the new authenticated snapshot arrives. Acceptance must change a project value in the dashboard, independently read it back through runtime-config, and verify the WordPress mirror and behavior. A runtime language change must remove only obsolete warmer pairs, URL work, oversize markers, and inactive domain-mapping payloads while preserving valid work. With automatic translation disabled, local and SaaS cache hits must still render, but misses must not spend fresh-word tickets or budgets; a source-language identity fallback under a target URL must emit non-cacheable headers before any full-page cache can store it.
