...
- **WordPress plugin signals**: a `deepglot_quota_exhausted` transient is set and a wp-admin notice is displayed when a live translation request returns 402; the dynamic-translator proxy also returns `quota_exhausted` so the browser client stops retrying for the session. (The health ping only reflects quota state via `connection_code: quota_exhausted` in the WordPress REST status response — it does not set the transient or trigger the admin notice.)
