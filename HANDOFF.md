...
- Branch: `main`
- WordPress plugin repository/release state: **v0.11.0 released on GitHub (2026-08-02, tag `wp-plugin-v0.11.0`, deterministic `deepglot-0.11.0.zip` + SHA-256 sidecar) — NOT deployed anywhere yet; `meinhaushalt.at` still runs v0.10.4 (verified via SSH on 2026-08-02).** The v0.10.4 → v0.11.0 delta is substantial: collision-safe translated URL slug mappings applied bidirectionally for path-prefix and subdomain routing ([#233](https://github.com/ostheimer/deepglot/pull/233)); preservation of special `mailto:`/`tel:` links and nav-switcher label fixes incl. Avada duplicate labels ([#234](https://github.com/ostheimer/deepglot/pull/234)); reproducible release tooling with truthful PHP 8.0 / GPL-2.0-or-later metadata, distributable readme, and hosted-service data-flow disclosure ([#235](https://github.com/ostheimer/deepglot/pull/235)); the Deepglot brand rollout incl. WP-admin branding and deepglot.ai onboarding links ([#236](https://github.com/ostheimer/deepglot/pull/236)); final release metadata ([#240](https://github.com/ostheimer/deepglot/pull/240)). Per the #233 deployment boundary, deployed customer sites need an explicit runtime sync afterwards before slug mappings take effect (noted there for the HD-Dental install).
- **Open follow-ups (2026-08-02):** unreviewed Codex draft PRs [#237](https://github.com/ostheimer/deepglot/pull/237) (post-merge review of #234: whitespace-prefixed `mailto:`/`tel:` values, Avada duplicate-label edge for `full_name`, `iso_code` label descriptions) and [#239](https://github.com/ostheimer/deepglot/pull/239) (post-merge review of #236: locale-aware `/manifest.webmanifest`, footer contrast, hero word-wrap). Assess #237 before deploying v0.11.0 to a live WordPress site; #239 is SaaS-side polish. [#222](https://github.com/ostheimer/deepglot/pull/222) (wordpress.org submission prep) remains open since 2026-07-16.
- WordPress plugin live state: **v0.10.4 deployed on `meinhaushalt.at` and origin/browser-verified (2026-07-27).** Server backup: `~/deepglot-backup-20260727090652-v0.10.3.tar.gz`; all 69 live PHP files linted cleanly. The origin `robots.txt` contains Yoast and Deepglot sitemap lines and now sends `no-cache, must-revalidate, max-age=0, no-store, private`. Browser verification on `/en/` confirmed `switcher.css?ver=0.10.4`, `<html lang="en">`, raw UTF-8, no numeric entities in `style`/`script`, and computed 🇬🇧/🇩🇪 pseudo-element content. **External cache boundary:** the public canonical `robots.txt` still serves the pre-deploy 30-day Cloudflare object until that exact URL is purged from the Cloudflare account that owns `meinhaushalt.at`.
...
- Purge Cloudflare's cached `https://www.meinhaushalt.at/robots.txt` object from the account that owns the zone, then verify that the public response advertises both Yoast's index and `/deepglot-sitemap.xml` without a long-lived cache header.
