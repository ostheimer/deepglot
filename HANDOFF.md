...
- Previous live state for reference: v0.10.4 (2026-07-27) with backup `~/deepglot-backup-20260727090652-v0.10.3.tar.gz`. **External cache boundary (as of 2026-07-27, unverified since):** the public canonical `robots.txt` may still serve the pre-deploy 30-day Cloudflare object until that exact URL is purged from the Cloudflare account that owns `meinhaushalt.at`.
