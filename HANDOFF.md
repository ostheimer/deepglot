...
- Branch: `main`
- WordPress plugin: **v0.8.6 remains deployed on `meinhaushalt.at` (2026-07-12); repository v0.10.0 is not deployed yet.** Live v0.8.6 was verified with versioned assets, translated human traffic, and cache-only/rejected unauthenticated dynamic requests. Repository v0.10.0 adds independent switcher instances/templates/visual placement plus AMP pipeline enforcement and the multilingual sitemap. Treat real WordPress visual, AMP, sitemap/robots, and mapped-subdomain checks as deployment gates before syncing production. The previous v0.8.6 deploy used a server backup, checksum-only rsync of three files, remote PHP syntax validation, and a WP Rocket page-cache purge; do not infer that the new repository files are live.
