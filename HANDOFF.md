...
- **Fix the batch-size / timeout mismatch (highest impact, see Current State for the measurements).** Options to weigh rather than apply blindly: lower `HtmlTranslator::BATCH_SIZE` so several small batches exist that the parallel path can actually parallelize; align the client timeouts with real API latency; and/or reduce SaaS response time for large batches. A cold page must not stay untranslated forever, and warming should not block the visitor's request for 15 s.
