...
- Branch: `main`
- WordPress plugin: **v0.10.3 deployed on `meinhaushalt.at` and origin-verified (2026-07-27); repository patch v0.10.4 awaits rollout.** v0.10.3 fixes the Yoast discovery ordering by appending `/deepglot-sitemap.xml` at priority 100,000. Origin verification contains both sitemap lines, but the canonical Cloudflare response still serves a pre-deploy, 30-day-cached `robots.txt` object. v0.10.4 sends no-cache headers for the virtual WordPress response; its rollout must also purge that one existing Cloudflare URL. The prior `/en/` browser check confirmed raw UTF-8 and computed 🇬🇧/🇩🇪 pseudo-element content.
...
- Deploy repository plugin v0.10.4 after CI, purge Cloudflare's cached `/robots.txt`, then verify that the public response advertises both Yoast's index and `/deepglot-sitemap.xml` without a long-lived cache header.
