...
- Branch: `main`
- WordPress plugin: **v0.8.6 deployed on `meinhaushalt.at` (2026-07-12)** — repo and live are in sync. Live-verified: assets serve `?ver=0.8.6` (`dynamic-translator.js` + `switcher.js`), the human path (`/en/`) renders translated (`<html lang="en">` + English content), and the dynamic-translate endpoint without a nonce/ticket degrades to cache-only — a browser-UA POST returns `{from_words:[],to_words:[]}` (HTTP 200, no spend, no 500) while a bot-UA POST is rejected with `403` at `permissionCheck`. Deployed via `rsync -ac --checksum` (only the 3 changed files: `deepglot.php`, `README.md`, `includes/Frontend/DynamicTranslationController.php`; `tests/` excluded, no `--delete`) after a server backup (`~/deepglot-plugin-backup-pre-086.tar.gz`) + remote `php -l` + WP Rocket page-cache purge. v0.8.6 (#212) adds a plugin-side budget rollback: on a failed SaaS call the plugin releases its ticket + per-IP fresh-word reservations instead of burning them.
