...
| 8.36 | **Dynamic-translate proxy: block quota drain via spoofed Origin** ([#199](https://github.com/ostheimer/deepglot/pull/199), plugin v0.8.4). `POST /wp-json/deepglot/v1/translate-dynamic` treated a scraped REST nonce plus a spoofable `Origin` header as proof of same-origin traffic, so server-side HTTP clients could burn project quota without a browser. Two independent caps now gate fresh-word spend: a short-lived **server-issued ticket** (minted per page render, so quota spend requires actually rendering the page and is capped per render), and — because page renders are free and mint unlimited tickets — a **per-IP fresh-segment window budget** (`MAX_FRESH_SEGMENTS_PER_IP`) that bounds total drain regardless of ticket count. The REST nonce alone degrades to cache-only. Test-first: `DynamicTranslationControllerTest` cases 6–7 (ticket required) and 15–17 (both caps + legitimate first-visit still served). | ✅ Completed |
