...
| 8.29 | **Guard dynamic API translations by origin** (PR #133). `/wp-json/deepglot/v1/translate-dynamic` validates the request origin server-side, enforcing the same-origin constraint independently of the client JS. Merged 2026-06-07. | ✅ Completed |
| 8.30 | **Fix property-only input value translation** (PR #134). Input elements whose only translatable content is an attribute value (e.g. `placeholder`, `aria-label`) were silently skipped by the dynamic pass. Fixed. Merged 2026-06-07. | ✅ Completed |
| 8.31 | **Sync dynamic translation setting** (PR #135). The `enable_dynamic_translation` admin toggle is now included in the plugin settings sync payload so the runtime configuration reflects the current admin choice. Merged 2026-06-07. | ✅ Completed |
| 8.32 | **Revalidate pending dynamic exclusions** (PR #136). Dynamic translation queue items re-check the current exclusion list on each sync cycle, discarding items that match a newly added exclusion rule. Merged 2026-06-07. | ✅ Completed |
