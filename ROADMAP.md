...
| 8.11 | **Fix project-create quota enforcement** — `POST /api/projects` still used a pre-`BILLING_PLANS` `planLimits` map (`PROFESSIONAL`/`ENTERPRISE` only), so paid tiers such as `PRO` fell through to the default of **1** project and blocked legitimate multi-project usage after Stripe checkout. Now uses `getEffectiveProjectsLimit()` (same soft-cap policy as word limits for non-ACTIVE subscriptions). Completed 2026-05-24. | ✅ Completed |
