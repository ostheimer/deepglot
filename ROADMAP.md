...
| 8.2 | **Switcher Weglot-parity remainder** ([#57](https://github.com/ostheimer/deepglot/issues/57)). Completed in repository plugin v0.10.0 on 2026-07-13: independently persisted switcher instances across shortcode/block/widget/automatic placement, safe migration of the former global configuration, three versioned templates, a same-origin script-free placement preview with conservative selector validation and footer fallback, unique ARIA state, and complete PHP/JavaScript regression coverage. A real non-production WordPress visual acceptance remains a deployment gate, not an implementation gap. | ✅ Completed |
| 8.3 | **Strategic Weglot competitive gaps** ([#58](https://github.com/ostheimer/deepglot/issues/58)). Completed or explicitly resolved on 2026-07-13: real visual-editor persistence/isolation acceptance; organization Translation Memory using only manager-approved segments; existing glossary dashboard; bounded text-based PDF translation with quota/velocity accounting; multilingual WordPress sitemap; and end-to-end AMP option enforcement. Translation CDN is deliberately deferred with the reverse-proxy/platform expansion in #121 until its documented customer-demand and security gates are met. | ✅ Completed |
...
| 8.36 | **Dynamic-translate proxy: mitigate quota drain via spoofed Origin** ([#199](https://github.com/ostheimer/deepglot/pull/199), plugin v0.8.4; ticket-debit fix v0.8.5). `POST /wp-json/deepglot/v1/translate-dynamic` treated a scraped REST nonce plus a spoofable `Origin` header as proof of same-origin traffic, so server-side HTTP clients could burn project quota without a browser. Replaced the spoofable Origin gate with two word-denominated caps: a short-lived **server-issued ticket** (minted per page render — quota spend now requires actually rendering the page, capped per render) and a **per-IP fresh-word window budget** (`MAX_FRESH_WORDS_PER_IP`). Test-first: `DynamicTranslationControllerTest` cases 6–7 (ticket required) and 15–18 (both caps, per-IP accumulation across tickets, legitimate first-visit still served). **v0.8.5 follow-up (case 19):** the per-IP budget is now reserved before the ticket, with a rollback (`releaseFreshWordsForIp`) if the ticket check fails — previously a per-IP rejection still debited the ticket budget, burning the per-render allowance for visitors behind shared NAT. **Interim mitigation only** — an adversarial review confirmed the caps are soft (non-atomic transients) and per-IP, so a distributed attacker rotating IPs can still burn the victim's remaining monthly quota early. The authoritative per-org fresh-word velocity limit landed SaaS-side in 8.37 ([#203](https://github.com/ostheimer/deepglot/issues/203)). | ✅ Completed |
| 8.37 | **SaaS-side per-org fresh-word velocity limit** ([#203](https://github.com/ostheimer/deepglot/issues/203)). The authoritative fix behind the plugin's soft per-IP caps (8.36): `POST /api/translate` now atomically reserves the fresh (provider-billed) word count against a per-**organization** fixed 1-hour-window budget via the existing `RateLimitBucket` upsert — a hard, atomic cap (closes the review's TOCTOU concern) keyed per org to match the per-org monthly quota (closes the IP-rotation bypass and prevents an org with N project keys from draining N× the rate against one shared pool). The default cap is 10% of the effective monthly quota with a 1,000-word minimum; `TRANSLATE_WORD_VELOCITY_PER_HOUR` remains an explicit operator override. Cache hits and bots are exempt; `quota_probe` is **not** (attacker-settable flag that still spends). Over-budget → `429 velocity_limited` + `Retry-After`. Monthly quota still caps the total; this caps the drain *rate*. Adversarial-reviewed. Test-first: `rate-limit.test.ts` (cost-based reservation, per-org isolation, window reset) + `translate-velocity-guard.test.ts` (route wiring, probes charged). | ✅ Completed |
| 8.38 | **Budget rollback on failed fresh-word spend** (plugin v0.8.6; SaaS translate route). After #210/#211 fixed pre-SaaS rejections poisoning local/SaaS velocity budgets, two post-reservation failure paths still debited without a successful spend: (1) the WP dynamic-translate proxy reserved per-IP and per-ticket budgets before calling SaaS but never rolled them back on 5xx/429/malformed responses — shared-NAT visitors lost their hourly allowance after transient SaaS errors; (2) `/api/translate` refunded velocity on provider failure but not when the persistence transaction failed after `translateTexts` succeeded. Fix: `releaseFreshWordBudget()` + rollback when `$fresh` stays empty; velocity refund in the DB-transaction `catch`. Test-first: `DynamicTranslationControllerTest` case 20 + `translate-velocity-guard.test.ts` persistence guard. | ✅ Completed |
| 8.39 | **Velocity-store parity and plan-derived cap** ([#214](https://github.com/ostheimer/deepglot/issues/214)). Exported the authoritative `PrismaRateLimitStore` for direct verification, added a memory/Prisma store-parity sequence plus a real-PostgreSQL concurrent reservation test, and changed `/api/translate` from the former flat 50k default to 10% of the organization's effective monthly quota with a 1,000-word minimum. A valid positive-integer `TRANSLATE_WORD_VELOCITY_PER_HOUR` still overrides the derived value; invalid overrides fall back safely. The route source guard pins the effective-quota wiring. | ✅ Completed |
| 8.40 | **`/api/translate` must invalidate stale workflow approval on machine re-translation** (cron bug scan 2026-07-14). PR #216 added organization Translation Memory that trusts only `APPROVED` segments from other projects, but `/api/translate` still updated `translatedText` without calling `resetTranslationWorkflowAfterContentEdit` when glossary invalidation or provider refresh forced a new machine translation. A previously approved segment could keep `APPROVED` while serving new machine text on the live site and propagating it cross-project via Translation Memory. Import and visual-editor writes already reset workflow; machine translate now matches. Guard extended in `translation-workflow-wiring.test.ts`. | ✅ Completed |
| 8.41 | **Import/visual-editor must not reset workflow on unchanged text** (cron bug scan 2026-07-19). #217 reset `APPROVED` → `ASSIGNED` on every PO/CSV re-import upsert even when `translatedText` was identical, so a routine re-sync of the same file wiped every approval. `workflowResetFieldsIfTranslatedTextChanged()` now gates the reset on a real content change for import and manual writes; machine translate still resets on provider refresh. | ✅ Completed |
| 8.42 | **Weekly workspace activity digest.** Opt-in preference per user/workspace, previous-complete-week aggregation for new translations/words, manual edits, and runtime translation requests, localized Cloudflare email template, quiet-week suppression, `CRON_SECRET`-protected Monday Vercel Cron, and unique per-recipient delivery claims with retry release on failed sends. Production schema, deployment, authenticated settings flow, real Cloudflare delivery to `office@ostheimer.at`, quiet-week suppression, and duplicate invocation were verified on 2026-08-04. The follow-up cron fix prevents a resend when the final `sentAt` observability write fails after provider acceptance by using a pending-send sentinel and retried completion write. | ✅ Completed |
| 8.43 | **Controlled WordPress URL synchronization (plugin v0.12.0).** Administrators preview and explicitly confirm an immutable batch of up to 250 safe internal URLs from the multilingual sitemap, then feed target-language pages into the existing TranslationWarmer without a permanent crawler. Source offsets continue large sites in bounded batches. The WP-Cron job opens at most two pages per run, applies queue and API-rate-limit backpressure, retry/backoff, HMAC-signed loopbacks, atomically locked pause/resume/cancel/failed-only-retry controls, and automatic quota/invalid-key pauses. Authenticated wp-admin and REST controls expose aggregate progress; a page leaves the queue only after its sync response reports zero pending segments for the expected language. | ✅ Completed |
