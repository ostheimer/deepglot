...
| 8.42 | **Weekly workspace activity digest.** Opt-in preference per user/workspace, previous-complete-week aggregation for new translations/words, manual edits, and runtime translation requests, localized Cloudflare email template, quiet-week suppression, `CRON_SECRET`-protected Monday Vercel Cron, and unique per-recipient delivery claims with retry release on failed sends. Cron pass 2026-08-04 fixed duplicate resend when `sentAt` observability write failed after provider acceptance (pending-send sentinel + retried completion write). | 🔄 Implemented; production schema sync, deploy, and live delivery acceptance pending |
