...
| 8.36 | **Dynamic-translate proxy: block quota drain via spoofed Origin** (cron bug scan). `POST /wp-json/deepglot/v1/translate-dynamic` treated a scraped REST nonce plus a spoofable `Origin` header as proof of same-origin traffic, so server-side HTTP clients could burn project quota without a browser. Quota spend now requires a short-lived server-issued ticket (one per page load, capped fresh words); the REST nonce alone degrades to cache-only. Test-first: `DynamicTranslationControllerTest` cases 6–7. | ✅ Completed |
