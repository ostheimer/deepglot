...
BASH;
    ...
    $guardedZip = $failingShaOutput . '/deepglot-0.12.7.zip';
...
if [[ ! -f "$output_directory/deepglot-0.12.7.zip" || -f "$output_directory/deepglot-0.12.7.zip.sha256" ]]; then
...
if [[ -e "$output_directory/.deepglot-0.12.7.lock" ]]; then
...
BASH;
    ...
    $parallelZip = $parallelOutput . '/deepglot-0.12.7.zip';
    packageAssert(
        file_get_contents($parallelZip . '.sha256') === hash_file('sha256', $parallelZip) . "  deepglot-0.12.7.zip\n",
    ...
    $firstZip = $fixtureRoot . '/dist-utc/deepglot-0.12.7.zip';
    $secondZip = $fixtureRoot . '/dist-honolulu/deepglot-0.12.7.zip';
    $firstChecksum = $fixtureRoot . '/dist-utc/deepglot-0.12.7.zip.sha256';
    ...
    $expectedChecksum = hash_file('sha256', $firstZip) . "  deepglot-0.12.7.zip\n";
