 ...
 *   - rocket_rucss_inline_atts_exclusions preserves the style element
 *     generated by wp_add_inline_style() for responsive and flag rules.
...
// -----------------------------------------------------------------------
// 4. The WordPress-generated inline style element is covered by the
//    attribute exclusion pattern.
// -----------------------------------------------------------------------
...
(new LanguageSwitcher($options, $routing))->enqueueStyles();
$dynamicCss = implode('', array_map(
    static fn (array $args): string => (string) ($args[1] ?? ''),
    $GLOBALS['_deepglot_inline_styles']
));
wprAssert(str_contains($dynamicCss, '@media'), 'Responsive CSS is attached through wp_add_inline_style');
wprAssert(str_contains($dynamicCss, '.deepglot-flag--en'), 'Flag CSS is attached through wp_add_inline_style');
...
$openingTag = '<style id="deepglot-switcher-inline-css">';
$covered = false;
...
wprAssert($covered, 'WordPress-generated inline style is covered by an attribute exclusion');
...
foreach ($contentPatterns as $pattern) {
    if (is_string($pattern) && $pattern !== '' && strpos($dynamicCss, $pattern) !== false) {
        $contentHit = true;
    }
}
