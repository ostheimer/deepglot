...
When every attempted SaaS provider returns only a count mismatch for the same multi-text chunk, Deepglot starts bounded binary isolation. The observed two-text case can recover both singletons. The provider-call ceiling is derived from the root chunk size and the configured provider chain: chain length × (2 × chunk size − 1); a default eight-text chunk with two providers therefore allows at most 30 provider calls. This ceiling covers the complete binary isolation tree and remains a backstop against recursion bugs. Provider calls retain the per-request concurrency cap, and all provider work shares a 100-second deadline. A failing parallel chunk stops further recursive calls, while the WordPress warmer keeps any terminal remainder queued. Singleton, depth-limit, call-budget, and deadline mismatches remain terminal; timeouts, authentication failures, rate limits, U+0000 output, and other malformed responses never enter this extra split path.
