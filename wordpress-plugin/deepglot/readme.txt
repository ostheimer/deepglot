...
When every attempted SaaS provider returns only a count mismatch for the same multi-text chunk, Deepglot isolates it for up to three binary levels. This can recover valid single texts from the default eight-text provider chunk while the WordPress warmer keeps any terminal remainder queued. Singleton and depth-limit mismatches remain errors; timeouts, authentication failures, rate limits, U+0000 output, and other malformed responses never enter this extra split path.
