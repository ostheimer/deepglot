...
**Access control:** Use `userCanManageProject()` for all write operations on projects. Use `userHasProjectAccess()` for read operations only. Both live in `src/lib/project-access-policy.ts`.

**Stripe safety:** Always call `isRealStripeCustomerId()` (in `src/lib/stripe.ts`) before any Stripe Customer API call. Synthetic/placeholder IDs must never reach the Stripe API.
