...
- Set `DEEPGLOT_SECRET_ENCRYPTION_KEY` (a 32-byte base64 secret, generated with `openssl rand -base64 32`) to use a dedicated AES-256 key for at-rest encryption of per-project custom provider API keys. Without it, `AUTH_SECRET` is used as the fallback encryption key — custom API keys are always encrypted, never stored in plaintext. **Key stability:** setting this variable on an existing deployment changes the decryption key for all previously stored `translationApiKeyEncrypted` values and will make them unreadable until re-entered. Set it once at initial install and do not rotate it without re-encrypting all stored keys.
