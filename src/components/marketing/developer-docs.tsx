    ...
    <div className="min-h-screen bg-[#fbfaf7] text-[#071521]">
      ...
      <main>
        ...
        <section id="wordpress-releases" className="scroll-mt-8 pt-20">
          ...
          <p className="mt-5 max-w-4xl leading-7 text-gray-700">
            {de
              ? "Die Releases sind als Git-Tags dokumentiert und bilden die Grundlage für die aktuelle v0.12.3-Implementierung. v0.12.1 vergab für den Retry-After-fähigen dynamischen Übersetzer eine neue öffentliche Asset-Version. v0.12.2 verifiziert bei der URL-Synchronisierung genau eine sichere kanonische Weiterleitung, ohne automatisches Folgen zu aktivieren. v0.12.3 speichert Text- und URL-Warteschlangen in einer versionierten, prüfsummengeschützten ASCII-Hülle, damit Emoji und anderes Vier-Byte-Unicode auch auf älteren WordPress-Optionstabellen erhalten bleiben; beschädigte Persistenz wird nicht still ersetzt. Eine eigene kurze atomare Sperre koppelt Text-, URL- und Purge-Abgleich, ohne Anbieteraufrufe einzuschließen. Ein GitHub-Release oder ZIP-Bau installiert kein Kunden-Plugin automatisch; produktive Installation und Live-QA bleiben getrennte Freigaben."
              : "These releases are documented as Git tags and form the basis for the current v0.12.3 implementation. v0.12.1 assigned a new public asset version to the Retry-After-aware dynamic translator. v0.12.2 verifies exactly one safe canonical redirect during URL synchronization without enabling automatic redirect following. v0.12.3 stores text and URL queues in a versioned, checksummed ASCII envelope so emoji and other four-byte Unicode remain durable on legacy WordPress option tables; damaged persistence is not silently replaced. A separate short atomic lock couples text, URL, and purge reconciliation without enclosing provider calls. A GitHub release or ZIP build does not automatically install a customer plugin; production installation and live QA remain separate approvals."}
          </p>
