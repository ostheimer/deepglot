    ...
    <div className="min-h-screen bg-[#fbfaf7] text-[#071521]">
      ...
      <main>
        ...
        <section id="wordpress-operations" className="scroll-mt-8 pt-20">
          ...
          <p className="mt-3 max-w-4xl leading-7 text-gray-700">
            {de
              ? "Melden alle versuchten Anbieter für denselben mehrteiligen Stapel ausschließlich eine Abweichung bei der Ergebnisanzahl, startet Deepglot eine begrenzte binäre Aufteilung. Der beobachtete Zwei-Text-Fall kann beide Einzeltexte erreichen. Die Obergrenze der Anbieteraufrufe ergibt sich aus der Größe des ursprünglichen Stapels und der konfigurierten Anbieterkette: Kettenlänge × (2 × Stapelgröße − 1). Beim Standardfall mit acht Texten und zwei Anbietern sind das höchstens 30 Anbieteraufrufe. Diese Grenze deckt den vollständigen binären Aufteilungsbaum ab und bleibt eine Absicherung gegen Rekursionsfehler. Anbieteraufrufe behalten die anfrageweite Parallelitätsgrenze; für alle Anbieterarbeiten gemeinsam gelten weiterhin höchstens 100 Sekunden. Bei PDF-Übersetzungen beginnt am Routeneintritt ein eigenes Budget von 40 Sekunden; Authentifizierung, Multipart-Verarbeitung und PDF-Vorbereitung werden davon abgezogen, damit für Abschlussarbeiten der 60-Sekunden-Route nominell 20 Sekunden bleiben. Fehler eines parallelen Stapels stoppen weitere rekursive Aufrufe. Abweichungen bei einem Einzeltext sowie am Tiefen-, Aufruf- oder Zeitlimit bleiben endgültige Fehler; Zeitüberschreitungen, Authentifizierungsfehler, Ratenlimits, U+0000 und andere ungültige Antworten lösen diese Zusatzanfragen nicht aus."
              : "When every attempted provider reports only a count mismatch for the same multi-text chunk, Deepglot starts bounded binary isolation. The observed two-text case can reach both singletons. The provider-call ceiling is derived from the root chunk size and the configured provider chain: chain length × (2 × chunk size − 1); a default eight-text chunk with two providers therefore allows at most 30 provider calls. This ceiling covers the complete binary isolation tree and remains a backstop against recursion bugs. Provider calls retain the per-request concurrency cap, and all provider work shares a 100-second deadline. PDF translation uses a separate 40-second budget from route entry; authentication, multipart handling, and PDF preparation reduce the remaining provider time so the 60-second route nominally retains 20 seconds for completion work. A failing parallel chunk stops further recursive calls. Singleton, depth-limit, call-budget, and deadline mismatches remain terminal; timeouts, authentication failures, rate limits, U+0000, and other malformed responses never trigger these extra requests."}
          </p>
          <p className="mt-3 max-w-4xl leading-7 text-gray-700">
            {de
