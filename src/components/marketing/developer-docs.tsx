    ...
    <div className="min-h-screen bg-[#fbfaf7] text-[#071521]">
      ...
      <main>
        ...
        <section id="wordpress" className="scroll-mt-8 pt-20">
          ...
          <p className="mt-3 max-w-4xl leading-7 text-gray-700">
            {de
              ? "Ab Plugin-Version 0.12.10 können Projektverwaltende vorhandene Medien-URLs je Zielsprache zuordnen: Bilder, PDF-, DOCX-, XLSX- und PPTX-Links, selbst gehostete MP4-/WebM-Videos sowie direkte YouTube- und Vimeo-Einbettungen. Die Zuordnung erfolgt über die angemeldeten Projekt-Medienrouten; eine Bearbeitungsoberfläche im Dashboard gibt es dafür derzeit nicht. Deepglot ersetzt passende URLs im serverseitig gerenderten WordPress-HTML. Dateien werden weder hochgeladen noch erzeugt; Rechte und Erreichbarkeit der Zielmedien sind vorab zu prüfen. Dynamisch nachgeladene Inhalte und vorgeschaltete CDN-Caches erfordern eine gesonderte Prüfung."
              : "From plugin version 0.12.10, project managers can map existing media URLs per target language: images, PDF, DOCX, XLSX, and PPTX links, self-hosted MP4 and WebM video, and direct YouTube and Vimeo embeds. Mappings use the authenticated project media routes; there is no dashboard editing interface yet. Deepglot replaces matching URLs in server-rendered WordPress HTML. It neither uploads nor creates files; check rights and target availability first. Dynamically inserted content and upstream CDN caches require separate verification."}
          </p>
          <p className="mt-3 max-w-4xl leading-7 text-gray-700">
            {de
              ? "Für Bild-, Dokument- und Videodateien sind nur root-relative URLs oder HTTPS-URLs auf dem exakten Projekthost erlaubt; dort sind Query-Parameter möglich. Dokumente und Videos behalten bei der Ersetzung ihre Dateiendung. Externe Einbettungen sind ausschließlich exakte HTTPS-URLs von www.youtube.com/embed/{ID}, www.youtube-nocookie.com/embed/{ID} oder player.vimeo.com/video/{ID}; der Zielhost muss derselbe Anbieterhost sein. Für Einbettungen sind Query-Parameter nicht erlaubt. Fragmente, YouTube-Watch-Links, Kurzlinks und externe CDNs werden abgewiesen."
              : "Image, document, and video files must use root-relative URLs or HTTPS URLs on the exact project host; those URLs may include query strings. Document and video replacements retain their file extension. External embeds are limited to exact HTTPS URLs at www.youtube.com/embed/{ID}, www.youtube-nocookie.com/embed/{ID}, or player.vimeo.com/video/{ID}; replacements keep the same provider host. Embeds cannot have query strings. Fragments, YouTube watch links, short links, and external CDNs are rejected."}
          </p>
          <p className="mt-3 max-w-4xl leading-7 text-gray-700 [overflow-wrap:anywhere]">
            {de
