...
export async function MarketingHome({ locale }: MarketingHomeProps) {
  ...
  return (
    <div className={`${manrope.className} min-h-screen bg-[#fbfaf7] text-[#071521]`}>
      ...
      <footer className="border-t border-[#d9dad7] bg-[#fbfaf7] py-12">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 md:flex-row sm:px-6 lg:px-8">
          <Link
            ...
            aria-label="Deepglot"
          >
            <Image src="/marketing/deepglot-mark.png" alt="" width={28} height={28} className="h-7 w-7" />
            <span className="font-bold text-[#071521]">Deepglot</span>
            <span className="ml-2 text-sm text-[#69737b]">
              © {new Date().getFullYear()} Andreas Ostheimer
            </span>
          </Link>
          <div className="flex gap-6 text-sm text-[#69737b]">
            <Link
              href={getMarketingPath(locale, "privacy")}
              className="transition-colors hover:text-[#f03b22]"
            >
              {copy.footer.privacy}
            </Link>
            <Link
