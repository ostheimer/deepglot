        ...
        <div className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
          ...
          <section id="wordpress-warmup" data-testid="help-wordpress-warmup" className="scroll-mt-8 pt-24">
            <div className="max-w-3xl">
              ...
              <p className="mt-4 text-sm leading-7 text-[#58636d]">
                {de
                  ? "Melden alle versuchten Anbieter bei einem mehrteiligen Stapel nur eine Abweichung bei der Ergebnisanzahl, startet Deepglot eine begrenzte binäre Aufteilung. Der beobachtete Zwei-Text-Fall kann beide Einzeltexte erreichen. Die Obergrenze der Anbieteraufrufe ergibt sich aus der Größe des ursprünglichen Stapels und der konfigurierten Anbieterkette: Kettenlänge × (2 × Stapelgröße − 1). Beim Standardfall mit acht Texten und zwei Anbietern sind das höchstens 30 Anbieteraufrufe. Diese Grenze deckt den vollständigen binären Aufteilungsbaum ab und bleibt eine Absicherung gegen Rekursionsfehler. Anbieteraufrufe behalten die anfrageweite Parallelitätsgrenze; für alle Anbieterarbeiten zusammen gelten weiterhin höchstens 100 Sekunden. Für PDF-Übersetzungen läuft ab Routeneintritt ein eigenes Budget von 40 Sekunden; Authentifizierung, Upload-Verarbeitung und PDF-Vorbereitung verkürzen die verbleibende Anbieterzeit, damit für Abschlussarbeiten der 60-Sekunden-Route nominell 20 Sekunden bleiben. Abweichungen bei einem Einzeltext sowie am Tiefen-, Aufruf- oder Zeitlimit bleiben endgültige Fehler. Fehler eines parallelen Stapels stoppen weitere rekursive Aufrufe; Zeitüberschreitungen, Authentifizierungsfehler, Ratenlimits, U+0000 und andere ungültige Antworten werden dadurch nicht zusätzlich wiederholt."
                  : "If every attempted provider reports only a count mismatch for a multi-text chunk, Deepglot starts bounded binary isolation. The observed two-text case can reach both singletons. The provider-call ceiling is derived from the root chunk size and the configured provider chain: chain length × (2 × chunk size − 1); a default eight-text chunk with two providers therefore allows at most 30 provider calls. This ceiling covers the complete binary isolation tree and remains a backstop against recursion bugs. Provider calls retain the per-request concurrency cap, and all provider work shares a 100-second deadline. PDF translation uses its own 40-second budget from route entry; authentication, upload handling, and PDF preparation reduce the remaining provider time so the 60-second route nominally keeps 20 seconds for completion work. Singleton, depth-limit, call-budget, and deadline mismatches remain terminal. A failing parallel chunk stops further recursive calls; timeouts, authentication failures, rate limits, U+0000, and other malformed responses are not retried by this extra isolation path."}
              </p>
              <p className="mt-4 text-sm leading-7 text-[#58636d]">
                {de
