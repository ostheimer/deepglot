          ...
          <section id="wordpress-warmup" data-testid="help-wordpress-warmup" className="scroll-mt-8 pt-24">
            <div className="max-w-3xl">
              ...
              <p className="mt-4 text-sm leading-7 text-[#58636d]">
                {de
                  ? "Sobald Warteschlange und fälliges Ereignis gespeichert sind, stößt Deepglot pro Anfrage einmal nicht blockierend WP-Cron an. Bei DISABLE_WP_CRON oder während eines Cron-Laufs bleibt dieser Anstoß aus. WP Rocket, W3 Total Cache und LiteSpeed Cache leeren fertig übersetzte URLs einzeln. WP Super Cache bietet nur einen globalen Purge; Deepglot wartet deshalb, bis die verfolgte Warteschlange leer ist, damit ausstehende Seiten im Cache bleiben. Fehlgeschlagene oder unvollständige Übersetzungen bleiben vorgemerkt und können bei einem späteren Cron-Lauf erneut versucht werden. Seit v0.12.3 bewahrt eine ASCII-sichere Warteschlange auch Emoji und anderes Vier-Byte-Unicode auf älteren WordPress-Datenbanken."
                  : "Once the queue and due event are stored, Deepglot makes one non-blocking WP-Cron nudge per request. That nudge is skipped for DISABLE_WP_CRON and while cron is already running. WP Rocket, W3 Total Cache, and LiteSpeed Cache purge completed URLs individually. WP Super Cache exposes only a global purge, so Deepglot waits until the tracked queue is empty and pending pages stay cached. Failed or partial translations remain queued for a later cron retry. Since v0.12.3, an ASCII-safe queue preserves emoji and other four-byte Unicode on legacy WordPress databases."}
              </p>
              <p className="mt-4 text-sm leading-7 text-[#58636d]">
                {de
                  ? "Melden alle versuchten Anbieter bei einem mehrteiligen Stapel nur eine Abweichung bei der Ergebnisanzahl, startet Deepglot eine begrenzte binäre Aufteilung. Der beobachtete Zwei-Text-Fall kann beide Einzeltexte erreichen; pro ursprünglichem Stapel gelten höchstens sechs Anbieteraufrufe und für alle Anbieterarbeiten zusammen höchstens 100 Sekunden. Für PDF-Übersetzungen läuft ab Routeneintritt ein eigenes Budget von 40 Sekunden; Authentifizierung, Upload-Verarbeitung und PDF-Vorbereitung verkürzen die verbleibende Anbieterzeit, damit für Abschlussarbeiten der 60-Sekunden-Route nominell 20 Sekunden bleiben. Ein weiterhin fehlerhafter Rest bleibt vorgemerkt. Fehler eines parallelen Stapels stoppen weitere rekursive Aufrufe; Zeitüberschreitungen, Authentifizierungsfehler, Ratenlimits, U+0000 und andere ungültige Antworten werden dadurch nicht zusätzlich wiederholt."
                  : "If every attempted provider reports only a count mismatch for a multi-text chunk, Deepglot starts bounded binary isolation. The observed two-text case can reach both singletons, but each original chunk allows at most six provider calls and all provider work shares a 100-second deadline. PDF translation uses its own 40-second budget from route entry; authentication, upload handling, and PDF preparation reduce the remaining provider time so the 60-second route nominally keeps 20 seconds for completion work. A terminal remainder stays queued. A failing parallel chunk stops further recursive calls; timeouts, authentication failures, rate limits, U+0000, and other malformed responses are not retried by this extra isolation path."}
              </p>
              <p className="mt-4 text-sm leading-7 text-[#58636d]">
                {de
